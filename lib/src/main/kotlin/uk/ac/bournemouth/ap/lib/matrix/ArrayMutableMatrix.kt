...
fun <T> MutableMatrix(original: Matrix<T>): MutableMatrix<T> {
    return MutableMatrix(original.width, original.height) { x, y ->
        original[x, y]
    }
}
...
abstract class ArrayMutableMatrixBase<T> protected constructor(
    ...
    override fun asSequence(): Sequence<T> {
        return (this as Iterable<T>).asSequence()
    }
...
class ArrayMutableSparseMatrix<T> : ArrayMutableMatrixBase<T>,
    ...
    private constructor(maxWidth: Int, data: Array<T?>, validator: (Int, Int) -> Boolean) :
            super(maxWidth, data) {
    ...
    override fun isValid(x: Int, y: Int): Boolean {
        return x in 0 until maxWidth && y in 0 until maxHeight && validator(x, y)
    }
