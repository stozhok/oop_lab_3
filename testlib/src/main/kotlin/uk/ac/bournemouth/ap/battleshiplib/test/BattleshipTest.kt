...
abstract class BattleshipTest {


    fun createOpponent(size: Size, ships: List<Ship>): BattleshipOpponent =
        createOpponent(size.columns, size.rows, ships)

    abstract fun createOpponent(columns: Int, rows: Int, ships: List<Ship>): BattleshipOpponent
    ...
    abstract fun createGame(grids: List<BattleshipGrid>): BattleshipGame

    @ParameterizedTest(name = "configuration: #{index}")
    @MethodSource("validShipConfigurations")
    open fun testValidShips(op: GameInfo) {
        val opponent = createOpponent(op.size, op.ships)
    ...
    open fun testCreateRandomOpponent(size: Size, shipSizes: IntArray, seed: Long) {
        val rnd = Random(seed)
    ...
    open fun testOpponentShipAt(opponentInfo: OpponentInfo) {
        val (size, ships, expectedGrid) = opponentInfo

        val opponent = createOpponent(size, opponentInfo.ships)
        ...
        expectedGrid.forEachIndex { x, y ->
            ...
            assertEquals(expectedGrid[x, y], actualShipIndex) {
                "Ship indices returned by shipAt in the opponent should match the ships passed in"
            }
        }
    ...
    open fun testInvalidShips(gameInfo: GameInfo) {
        ...
        val e = assertThrows<Exception> {
            val o = createOpponent(size, ships)
    ...
    open fun testCreateGrid(size: Size, shipSizes: IntArray, seed: Long) {
    ...
    open fun testShootCell(opponentInfo: OpponentInfo, seed: Long) {
        val rnd = Random(seed)
        val (size, ships, expected) = opponentInfo

        val opponent = createOpponent(size, ships)
    ...
    @ParameterizedTest(name = "configuration: ({0}) #{2}")
    @MethodSource("shotTestData")
    open fun testShootMisses(opponentInfo: OpponentInfo, seed: Long) {
        val (size, ships, expected) = opponentInfo
    ...
    @ParameterizedTest(name = "configuration: ({0}) #{2}")
    @MethodSource("shotTestData")
    open fun testShootEntireShip(opponentInfo: OpponentInfo, seed: Long) {
    ...
    open fun testPlayMaxFullGame(gameInfo: GameInfo) {
    ...
    open fun testPlayRegularGame(gameInfo: GameInfo) {
        val (size, ships, moves) = gameInfo
        val opponent = createOpponent(size, ships)
    ...
    @ParameterizedTest(name = "configuration: ({0}) #{2}")
    @MethodSource("normalGames")
    open fun testListeners(gameInfo: GameInfo) {
        val (size, ships, moves) = gameInfo
        val opponent = createOpponent(size, ships)
        ...
        assertMovesValid(grid, moves.dropLast(1)) { c, r ->
            ...
            assertFalse(testListener.isFinished, "The game should not be finished before the last move")
        }
    ...
    open fun testDisallowDuplicates(gameInfo: GameInfo, rnd: Random) {
        val grid = createGrid(createOpponent(gameInfo.size, gameInfo.ships))
    ...
    open fun testDisallowOutOfBounds(gameInfo: GameInfo) {
        val grid = createGrid(createOpponent(gameInfo.size, gameInfo.ships))
    ...
    open fun testPlayMinFullGame(opponentInfo: GameInfo) {
    ...
    companion object {
                    ...
                    InputStreamReader(inStream).useLines { lines ->
                        lines
                            ...
                            .map { line ->
                                ...
                                val size = Size(elems.next(), elems.next())
                                val shipData =
                                    IntMatrix(size.columns, size.rows) { _, _ -> elems.next() + 0 }
        ...
        fun gamesWithRnd() = Random(7523).let { rnd ->
            (minFullGames() + maxFullGames() + normalGames()).map { arrayOf(it, Random(rnd.nextLong())) }
        }
